...
| `APPKIT_TOKEN` secret | Fine-grained PAT (Contents: read) scoped to `HjtDev/appkit`, configured 2026-08-29. Used by CI's `backend-quality`, `backend-tests`, and `docker-build` jobs to fetch appkit's private backend repo — see `INTEGRATION-GUIDE.md` §2 step 2 and `.github/workflows/ci.yml`'s top comment. **Expires 2026-11-27 (90 days out).** Past that date CI fails at `uv sync`/`docker buildx build` with an authentication error, not an obviously-expired-token one — rotate it (Settings → Secrets and variables → Actions) before then, and update this date |
